...
namespace CryptoCompare.Tests
{
    public class HistoricaldataTests
    {
        ...
        {
            var response =
                await _cryptoCompareClient.HistoricalDataClient.GetHistoricalDayAveragePrice("BTC", "USD",null,null,null,"1487116600");
        ...
        public async Task Historical_Day_Exchange_Volume()
        {
            var limit = 10;
            var response =
                await _cryptoCompareClient.HistoricalDataClient.GetHistoricalDailyExchangeVolume("USD", null, null, limit);
        ...
        public async Task Historical_Hourly_Exchange_Volume()
        {
            var limit = 10;
            var response =
                await _cryptoCompareClient.HistoricalDataClient.GetHistoricalHourlyExchangeVolume("USD", null, null, limit);
            Assert.Equal("Got the data",response.Message);
            Assert.Equal(limit+1,response.Data.Length);
        }
    }
